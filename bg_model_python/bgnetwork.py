from main_simulator import * # Variables
from functions import creatdbs, calculateEI
from gating import *

def bgnetwork(pd, wstim, freq):
    # Membrane parameters
    ...
    gsyn = [1, 0.3, 1, 0.3, 1, .08]
    Esyn = [-85, 0, -85, 0, -85, -85]
    tau = 5
    gpeak = 0.43
    gpeak1 = 0.3
    ...
    vth = np.zeros(t.size) # thalamic membrane voltage
    vsn = np.zeros(t.size) # STN membrane voltage
    vge = np.zeros(t.size) #  GPe membrane voltage
    vgi = np.zeros(t.size) # GPi membrane voltage
    S2 = np.zeros(1)
    S21 = np.zeros(1)
    S3 = np.zeros(1)
    S31 = np.zeros(1)
